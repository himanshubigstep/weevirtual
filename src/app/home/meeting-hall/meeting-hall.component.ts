...
export class MeetingHallComponent implements OnInit {
  ...
  playSelfStream(stream_type){
    console.log("playselfstreamfunctioncalled")
    if (
  ...
  private assignClientHandlers(): void {
    ...
    this.client.on(ClientEvent.RemoteStreamSubscribed, (evt) => {
      ...
      console.log(typeof stream.getId())
      let remoteStreamId = stream.getId();

      if(typeof stream.getId()== 'string'){
        const stream_id = stream.getId().toString();

        if(stream_id.includes('-screen')){
          remoteStreamId = parseInt(stream_id.split('-screen')[0])
        }        
      }

      console.log(
        this.panellistUsersList.filter((res) => res.id == remoteStreamId)
      );
      
      const selectedObj = this.panellistUsersList.filter(
        (res) => res.id == remoteStreamId
      )[0];
      //if (selectedObj) {
  ...
  shareScreen(): void {
    ...
    var userID = this.uid+'-screen'; // set to null to auto generate uid on successfull connection
  ...
  protected initScreenShare(): void {
    ...
    this.screenStream.init(
      () => {
        ...
        console.log(this.screenStream.getId())
        if(user_id != this.screenStream['params'].streamID){
          console.log("playselfscreenstream")
  ...
  private assignSessionClientHandlers(): void {
    ...
    this.sessionClient.on(ClientEvent.RemoteStreamSubscribed, (evt) => {
      ...
      if(typeof stream.getId()== 'string'){
        ...
        if(stream_id.includes('-session-screen')){
          remoteStreamId = parseInt(stream_id.split('-session-screen')[0])
        }
        
        if(stream_id.includes('_session')){
          remoteStreamId = parseInt(stream_id.split('_session')[0])
        }
